...
class hmt_grn(nn.Module):
    ...
    def forward(self, input, mode, arg):
        ...
        if mode == 'train':
            ...
            x, users, y = LT(x).to(t.device("mps")), LT(users).to(t.device("mps")), LT(y).to(t.device("mps"))
            ...
            x, users, x_geoHash2, x_geoHash3, x_geoHash4, x_geoHash5, x_geoHash6 = LT(x).to(t.device("mps")), LT(users).to(t.device("mps")), LT(
            x_geoHash2).to(t.device("mps")), LT(x_geoHash3).to(t.device("mps")), \
            LT(x_geoHash4).to(t.device("mps")), LT(x_geoHash5).to(t.device("mps")), LT(x_geoHash6).to(t.device("mps"))
        ...
        finalAttendedOutSpatial = FT([]).to(t.device("mps"))
        finalAttendedOutTemporal = FT([]).to(t.device("mps"))
        ...
        for eachBatchIndex in range(batchSize):
            ...
            singleBatchSpatial = FT([]).to(t.device("mps"))
            singleBatchTemporal = FT([]).to(t.device("mps"))
            for timeStep in range(len(currentSample)):
                ...
                x_embed = self.poiEmbed(LT([sample]).to(t.device("mps")))

                if sample == 0: # padding
                    spatial_GAT_out = x_embed
                    temporal_GAT_out = x_embed
                else:
                    spatialNieghEmbed = self.poiEmbed(LT(spatialNiegh).to(t.device("mps"))).unsqueeze(0)
                    temporalNieghEmbed = self.poiEmbed(LT(temporalNiegh).to(t.device("mps"))).unsqueeze(0)
...
class multiHeadAttention(nn.Module):
    def __init__(self, arg, type):
        ...
        for i in range(self.numHeads):
            self.heads[i] = selfAttention(arg, type).to(t.device("mps")).float()

    def forward(self, mainNodeEmbed, neighbourNodesEmbeds, numTimeSteps, users, mode, arg):

        allHeads = FT([]).to(t.device("mps"))
...
class LSTMCell(nn.Module):
    ...
    def _init_hidden(self, x):
        ...
        h = t.zeros([batchSize, self.hidden_size]).to(t.device("mps"))
        c = t.zeros([batchSize, self.hidden_size]).to(t.device("mps"))
        return h, c

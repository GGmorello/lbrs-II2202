...
if __name__ == '__main__': 
    ...
    for dataSource in ['global_scale']:
        ...
        classification = hmt_grn(arg).float().to(t.device("mps"))
        ...
        for epoch in range(1, arg['epoch'] + 1):
            ...
            for x, user, y in classification_pbar:
                ...
                x_geoHash2 = LT([]).to(t.device("mps"))
                x_geoHash3 = LT([]).to(t.device("mps"))
                x_geoHash4 = LT([]).to(t.device("mps"))
                x_geoHash5 = LT([]).to(t.device("mps"))
                x_geoHash6 = LT([]).to(t.device("mps"))
                for eachBatch in range(x.shape[0]):
                    ...
                    x_geoHash2 = t.cat((x_geoHash2,LT(mappedGeohash).unsqueeze(0).to(t.device("mps"))),dim=0)

                    mappedGeohash = [arg['geohash2Index' + '_3'][arg['poi2geohash' + '_3'][i]] for i in sample]
                    x_geoHash3 = t.cat((x_geoHash3, LT(mappedGeohash).unsqueeze(0).to(t.device("mps"))), dim=0)

                    mappedGeohash = [arg['geohash2Index' + '_4'][arg['poi2geohash' + '_4'][i]] for i in sample]
                    x_geoHash4 = t.cat((x_geoHash4, LT(mappedGeohash).unsqueeze(0).to(t.device("mps"))), dim=0)

                    mappedGeohash = [arg['geohash2Index' + '_5'][arg['poi2geohash' + '_5'][i]] for i in sample]
                    x_geoHash5 = t.cat((x_geoHash5, LT(mappedGeohash).unsqueeze(0).to(t.device("mps"))), dim=0)

                    mappedGeohash = [arg['geohash2Index' + '_6'][arg['poi2geohash' + '_6'][i]] for i in sample]
                    x_geoHash6 = t.cat((x_geoHash6, LT(mappedGeohash).unsqueeze(0).to(t.device("mps"))), dim=0)
                ...
                for eachGeoHashPrecision in [6,5, 4, 3, 2]:
                    ...
                    truthDict[name] = LT([]).to(t.device("mps"))
                    for eachBatch in range(truth.shape[0]):
                        ...
                        truthDict[name] = t.cat((truthDict[name],LT(mappedNextGeohashTruth).unsqueeze(0).to(t.device("mps"))),dim=0)
